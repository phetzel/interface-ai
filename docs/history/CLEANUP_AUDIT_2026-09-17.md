...
[m3-check](https://github.com/phetzel/interface-ai/blob/613ee9cec4868be03dacd606de89e5e88dce5e4e/scripts/m3-check) hashes the host source before and after its run, but runs unit tests and desktop actions against an existing `interface-ai-desktop:local` image. An unchanged source tree is not proof that image contains that tree. [m1-check](https://github.com/phetzel/interface-ai/blob/06cfc07311e156e50e8d4ea0cb1e76ec2ce96c1d/scripts/m1-check) does compare many engine/infra/capability runtime files, but its mapping excludes the compiled React fixture. Recording a fixture image ID beside source hashes does not establish the relationship between them. The launcher intentionally reuses existing images; this is documented, but easy to miss while editing.
