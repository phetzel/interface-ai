...
Example goal: `Read the current savings account balance for member ID 00456`. This is the same intentionally bounded request grammar as the CLI. It is not arbitrary task understanding. The model chooses actions from screenshots; local typed checks determine success.
