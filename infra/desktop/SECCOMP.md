...
The upstream profile uses a default-deny syscall policy and permits `clone`, `setns`, and `unshare` for browser user namespaces. See [Playwright's Docker guidance](https://playwright.dev/docs/docker). Only the desktop service uses this profile; fixture and operator relay retain their existing settings.
