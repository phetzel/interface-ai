...
for (const expected of oracle.successes) {
  test(`savings lookup preserves identity ${expected.memberId}`, async ({
    page,
  }, info) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Member overview" }),
    ).toBeVisible();
    await expect(
      page.getByRole("region", { name: "Member identity" }),
    ).toContainText(expected.memberName);
    await expect(
      page.getByRole("region", { name: "Member identity" }),
    ).toContainText(expected.memberId);
    await expect(
      page.getByRole("button", { name: "View checking", exact: true }),
    ).toHaveCount(1);
    ...
    await page
      .getByRole("button", { name: "View savings", exact: true })
      .click();
    await expect(
      page.getByRole("heading", { name: `${expected.accountType} account` }),
    ).toBeVisible();
    await expect(
      page.getByText(expected.displayBalance, { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("region", { name: "Member identity" }),
    ).toContainText(expected.memberId);
    await expect(
      page.getByText(`•••• ${expected.lastFour}`, { exact: true }),
    ).toBeVisible();
    await expect(page.locator("dd").filter({ hasText: /^USD$/ })).toBeVisible();
    const box = await page
      .getByText(expected.displayBalance, { exact: true })
      .boundingBox();
...
test("return navigation and another member never retain the old balance", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect(
    page.getByText(oracle.successes[0].displayBalance, { exact: true }),
  ).toHaveCount(0);
  ...
  await page.getByRole("button", { name: "View savings" }).click();
  await expect(
    page.getByText(oracle.successes[1].displayBalance, { exact: true }),
  ).toBeVisible();
  await expect(
    page.getByText(oracle.successes[0].displayBalance, { exact: true }),
  ).toHaveCount(0);
  await page.reload();
  await expect(
    page.getByRole("heading", { name: "Find a member" }),
  ).toBeVisible();
  await expect(page.getByLabel("Member ID", { exact: true })).toHaveValue("");
});

test("missing member is explicit and a later valid search recovers", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect(page.getByRole("button", { name: "View savings" })).toHaveCount(
    0,
  );
  await search(page, oracle.successes[0].memberId);
  await expect(
...
test("invalid IDs are rejected without trimming, coercion, or silent padding", async ({
  page,
}) => {
  await page.goto("/");
  for (const memberId of oracle.invalidIds) {
    ...
    await expect(page.getByRole("button", { name: "Searching…" })).toHaveCount(
      0,
    );
  }
});
...
test("cancelled delayed search cannot overwrite a fresh search screen", async ({
  page,
}) => {
  await page.goto("http://127.0.0.1:4181/");
  await search(page, oracle.successes[0].memberId);
...
test("blocked search never reaches a result and can be reset", async ({
  page,
}) => {
...
test("duplicate scenario presents two indistinguishable savings targets", async ({
  ...
  await expect(
    region.getByRole("button", { name: "View savings", exact: true }),
  ).toHaveCount(2);
  await expect(
...
test("unreadable output never presents a numerical savings balance", async ({
  page,
}, info) => {
  ...
  await expect(
    page.getByRole("region", { name: "Member identity" }),
  ).toContainText(oracle.successes[0].memberId);
...
test("only launch-time scenario controls apply; oracle and source are not served", async ({
  ...
  expect(
    (await request.get("/fixture-config.json")).headers()["cache-control"],
  ).toBe("no-store");
...
test("configuration fetch failure does not silently use the default fixture", async ({
  page,
}) => {
  await page.route("**/fixture-config.json", (route) =>
