...
try {
  ...
  await writeFile(resolve(output, 'summary.json'), JSON.stringify({ status: 'passed', provenance: 'automated-operator-ui-test', viewport: [1000, 1200], scaledDesktopClicks: true, prematureResumeRejected: true, completed: true, pageErrors: 0, modelCalls: 0 }, null, 2) + '\n');
} finally {
  await browser.close();
}
