...
const clickDesktop = async (x, y) => {
  ...
  await page.mouse.click(box.x + 3 + x / 1280 * (box.width - 6), box.y + 3 + y / 800 * (box.height - 6));
...
try {
  ...
  if (!(await page.locator('#diagnostic').innerText()).includes('search-member') ||
      !(await page.locator('#diagnostic').innerText()).includes('intervention_required')) throw Error('Missing pause diagnostic');
  ...
  await page.getByRole('button', { name: 'Verify & resume', exact: true }).click();
  await state('success');
  if (!(await page.locator('#diagnostic').innerText()).includes('read-balance')) throw Error('Missing final step');
  ...
  await writeFile(resolve(output, 'summary.json'), JSON.stringify({ status: 'passed', provenance: 'automated-operator-ui-test', viewport: [1000, 1200], scaledDesktopClicks: true, prematureResumeRejected: true, stepAndReasonVisible: true, completed: true, pageErrors: 0, modelCalls: 0 }, null, 2) + '\n');
} finally {
  await browser.close();
}
