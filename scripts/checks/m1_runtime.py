...
for name, schema in exports.items():
    assert json.loads(Path('/opt/capabilities/schemas', name+'.schema.json').read_text()) == schema
validator = TypeAdapter(RunResult)
for name in manifest.get('results', []):
    validator.validate_json(Path(name).read_text(), strict=True)

assert os.getuid() != 0
assert not any('OPENAI' in name.upper() for name in os.environ), 'Unexpected OpenAI environment configuration'
packages = {dist.metadata['Name'].lower(): dist.version for dist in importlib.metadata.distributions()}
...
for address in ('1.1.1.1', '8.8.8.8', '2606:4700:4700::1111'):
    try:
        ...
        probes.append({'destination': address, 'port': 443, 'blocked': True, 'reason': type(exc).__name__})
    else:
        connection.close()
...
if lookup.returncode == 0:
    ...
    for address in addresses:
        try:
            ...
            probes.append({'destination': 'api.openai.com', 'resolvedAddress': address, 'port': 443,
                           'blocked': True, 'reason': type(exc).__name__})
        else:
            connection.close()
            raise AssertionError('Model endpoint TCP probe unexpectedly connected')
else:
    probes.append({'destination': 'api.openai.com', 'blocked': True, 'reason': 'dns_resolution_failed'})

print(json.dumps({'status': 'passed', 'architecture': platform.machine(), 'python': platform.python_version(),
                  'runtimeFileHashesChecked': len(manifest['runtimeFiles']), 'schemasMatchModels': len(exports),
                  'resultsValidated': len(manifest.get('results', [])), 'nonRoot': True,
                  'noOpenAIConfiguration': True, 'noModelSDK': True, 'modelCalls': 0,
                  'fixtureHealthReachable': True, 'noDefaultIPv4Route': True, 'tcpProbes': probes,
                  'packages': packages,
                  'scope': 'Selected IPv4, IPv6 and model endpoint probes; not a comprehensive network policy audit'}, indent=2))
