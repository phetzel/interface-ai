...
def screen_text(backend):
    ...
    result = subprocess.run(['tesseract','stdin','stdout','--psm','6'], input=buffer.getvalue(), capture_output=True, timeout=3, check=True)
...
try:
    ...
    denied = 0
    for method,path in [('GET','/transfer'),('POST','/'),('GET','/?secret=SECRET-SENTINEL-84729'),
                        ('GET','/%2e%2e/'),('GET','/assets/../index.html')]:
        try:
            urllib.request.urlopen(urllib.request.Request('http://fixture:4173'+path,method=method),timeout=3)
    ...
    with Desktop(session['id'], backend=backend, event_sink=events.append) as desktop:
        ...
        for action in ({'type':'click','x':1070,'y':710}, {'type':'hotkey','keys':['ctrl','l']},
                       {'type':'type','text':'SECRET-SENTINEL-84729'}, {'type':'press','key':'tab'}):
            try:
        ...
        assert ImageChops.difference(before,desktop.screenshot().crop((946,730,1220,755))).getbbox() is None
        passed('visible_risky_control_and_page_instruction_rejected', denied=len(events), inputActionsCompleted=0)
    ...
    with Desktop(session['id'], calibration=True) as calibration:
        ...
        wait(lambda: 'no transfer requested' not in screen_text(calibration.backend)
    ...
    capture=subprocess.run([sys.executable,'/opt/desktop/control.py','screenshot'],capture_output=True,timeout=5)
    ...
    try:
        for name,url in [('external','http://example.com/'),('file','file:///etc/passwd')]:
            ...
            subprocess.run(['chromium','--user-data-dir=/tmp/interface-ai/chromium-profile','--new-window',url],
