...
try {
  ...
  await page.mouse.click(box.x+3+600/1280*(box.width-6), box.y+3+390/800*(box.height-6));
  await poll(() => page.locator('#send').isEnabled());
  await page.locator('#text').fill('x'.repeat(256));
  const inputRequest = page.waitForRequest(r => r.url().endsWith('/action') && r.method()==='POST');
  let inputFinished = false;
  const inputResponse = page.waitForResponse(r => r.url().endsWith('/action') && r.request().method()==='POST')
    .then(response => { inputFinished=true; return response; });
  ...
  for (const id of ['send','resume','takeover','start']) assert.equal(await page.locator('#'+id).isEnabled(), false);
  await page.reload();
  await phase('stopped · stopped');
  assert.equal(await page.locator('#send').isEnabled(), false);
  assert.equal(await page.locator('#resume').isEnabled(), false);
  await poll(() => page.locator('#screen').evaluate(img => img.complete && img.naturalWidth===1280));
  ...
  await writeFile(resolve(output,'summary.json'), JSON.stringify({
    status:'passed', provenance:'automated-operator-ui-test', realHumanWitnessed:false,
    panelStopDuringPendingInput:true, inputRejectedAsStopped:true, stopResponseMs,
    sameSession:true, stoppedAfterReload:true, controlsDisabled:true, pageErrors:0, modelCalls:0,
  }, null, 2)+'\n');
} finally {
  await browser.close();
}
