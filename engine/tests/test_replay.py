...
BUNDLE_PATH = Path(__file__).resolve().parents[2] / 'capabilities/poc/savings-balance/capability.json'
...
class ContractTests(unittest.TestCase):
    ...
    def test_manual_artifact_round_trip_and_bound_assets(self):
        ...
        self.assertEqual(Capability.model_validate(bundle.capability.model_dump()), bundle.capability)
    ...
    def test_invalid_structures_versions_references_and_actions(self):
        ...
        def change(operation):
            cap = copy.deepcopy(self.raw); operation(cap); cases.append(cap)
        ...
        change(lambda c:c['targets']['search-heading'].update(threshold=.1))
        ...
        for case in cases:
            with self.subTest(case=cases.index(case)):
                with self.assertRaises(ValidationError): Capability.model_validate(case)
    ...
    def test_unknown_parent_dependency_rejected_without_keyerror(self):
        ...
        with self.assertRaises(ValidationError): Capability.model_validate(self.raw)
    ...
    def test_invalid_inputs_reject_without_coercion_or_raw_value(self):
        for value in [123, None, '', '123', '00123 ', '00123\n', '００１２３', 'sentinel']:
            with self.assertRaises(ReplayError) as error: validate_inputs({'memberId':value})
            self.assertEqual(error.exception.code,'invalid_input')
            self.assertNotIn('sentinel',str(error.exception))
        with self.assertRaises(ReplayError): validate_inputs({'memberId':'00340','other':True})

    def test_asset_tampering_and_symlink_escape_rejected(self):
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory); target=root/'bundle'
            ...
            with self.assertRaises(ReplayError) as error: load_bundle(target/'capability.json')
            self.assertEqual(error.exception.code,'invalid_asset')
            outside=root/'outside.png';outside.write_bytes(original)
            asset.unlink();asset.symlink_to(outside)
            with self.assertRaises(ReplayError): load_bundle(target/'capability.json')

    def test_preflight_rejects_before_desktop_acquisition(self):
        from interface_ai.replay.command import replay
        with tempfile.TemporaryDirectory() as directory:
            root=Path(directory)
            invalid=copy.deepcopy(self.raw);invalid['steps'][0]['action']='shell'
            bad=root/'invalid.json';bad.write_text(json.dumps(invalid))
            for capability,member in [(BUNDLE_PATH,'123'),(bad,'00340')]:
                args=SimpleNamespace(capability=str(capability),member_id=member,inputs_json=None,session=None)
                with patch('interface_ai.replay.command.Desktop') as desktop, redirect_stdout(StringIO()):
            ...
            for path in reports:
                ...
                self.assertEqual(report['phase'],'preflight');self.assertEqual(report['actionsCompleted'],0)


class FakeDesktop:
    def __init__(self, *, missing=False, initial='search', stop_after=None, fail_action=None):
        self.stage,self.missing=initial,missing
        self.actions=[];self.stop_after=stop_after;self.fail_action=fail_action
    def screenshot(self):return Image.new('RGB',(1280,800))
    def checkpoint(self):
        if self.stop_after is not None and len(self.actions)>=self.stop_after:
            raise DesktopError('stopped','stopped')
    def execute(self, action):
        self.actions.append(action)
        if len(self.actions)==self.fail_action:raise DesktopError('input_failed','input failed')
        if action['type']=='type':self.stage='entered'
        if action['type']=='press':self.stage='missing' if self.missing else 'member'
        if action['type']=='click' and self.stage=='member':self.stage='account'


class FakeObservation:
    def __init__(self, runner, image):self.runner,self.image=runner,image
    def checkpoint(self,name):
        stage=self.runner.desktop.stage
        return name in {'search':['search-ready'],'entered':['search-ready','input-entered'],
                        'member':['member-ready'],'account':['account-ready'],'missing':['member-not-found']}.get(stage,[])
    def target(self,name):return Box(100,100,120,120)
    def field(self,name):
        return {'account-member-id':self.runner.inputs.memberId,'member-name':'Synthetic Person',
                'account-type':'Savings','currency':'USD','balance':1203,'alternate-balance':4567}[name]
...
class InterpreterTests(unittest.TestCase):
    ...
    def run_with(self, desktop, bundle=None, observer=FakeObservation, clock=None):
        ...
        return Interpreter(bundle or self.bundle,MemberInput(memberId='00340'),desktop,event_sink=self.events.append,
                           observer_factory=observer,ocr=object(),**options).run()

    def test_success_binds_input_and_keeps_values_out_of_events(self):
        desktop=FakeDesktop();result=self.run_with(desktop)
        self.assertEqual(result.status,'success');self.assertEqual(result.output.amountMinor,1203)
        self.assertEqual(desktop.actions[2],{'type':'type','text':'00340'})
        self.assertNotIn('00340',json.dumps(self.events));self.assertNotIn('Synthetic Person',json.dumps(self.events))
    ...
    def test_artifact_changes_execution_sequence_and_output_binding(self):
        ...
        desktop=FakeDesktop();result=self.run_with(desktop,bundle)
    ...
    def test_known_outcome_exits_before_account_click_and_extraction(self):
        desktop=FakeDesktop(missing=True);result=self.run_with(desktop)
        self.assertEqual(result.status,'business_outcome');self.assertEqual(result.outcome,'member_not_found')
        self.assertEqual(len(desktop.actions),4);self.assertFalse(hasattr(result,'output'))

    def test_precondition_failure_dispatches_nothing(self):
        desktop=FakeDesktop(initial='unknown');result=self.run_with(desktop)
        self.assertEqual(result.code,'precondition_failed');self.assertEqual(desktop.actions,[])

    def test_uncertain_input_failure_is_never_retried(self):
        desktop=FakeDesktop(fail_action=3);result=self.run_with(desktop)
        self.assertEqual(result.code,'input_failed');self.assertEqual(len(desktop.actions),3)

    def test_stop_after_dispatch_prevents_any_subsequent_action(self):
        desktop=FakeDesktop(stop_after=3);result=self.run_with(desktop)
        self.assertEqual(result.code,'stopped');self.assertEqual(len(desktop.actions),3)

    def test_output_identity_mismatch_is_not_returned_as_success(self):
        class WrongIdentity(FakeObservation):
            def field(self,name):return '00912' if name=='account-member-id' else super().field(name)
        result=self.run_with(FakeDesktop(),observer=WrongIdentity)
        self.assertEqual(result.code,'identity_mismatch');self.assertFalse(hasattr(result,'output'))

    def test_wrong_output_type_is_rejected(self):
        class WrongOutput(FakeObservation):
            def field(self,name):return '1203' if name=='balance' else super().field(name)
    ...
    def test_multiple_matching_postconditions_are_ambiguous(self):
        class Ambiguous(FakeObservation):
            def checkpoint(self,name):return (name=='member-not-found' and self.runner.desktop.stage=='member') or super().checkpoint(name)
        desktop=FakeDesktop();result=self.run_with(desktop,observer=Ambiguous)
        self.assertEqual(result.code,'ambiguous_checkpoint');self.assertEqual(len(desktop.actions),4)

    def test_missing_postcondition_times_out_without_a_later_action(self):
        desktop=FakeDesktop();now=[0]
        def clock():
            if desktop.stage=='member':now[0]+=1
            return now[0]
