...
class Backend:
    ...
    def screenshot(self):
        return SimpleNamespace(size=self.dimensions)
    ...
    def type_character(self, text):
        self.record('type', text)
...
class DesktopTests(unittest.TestCase):
    ...
    def desktop(self, **kwargs):
        return Desktop(
            ...
            session_reader=lambda: dict(self.current),
            stop_path=self.stop,
    ...
    def test_all_invalid_arguments_rejected_before_input(self):
        actions = [
            None,
            {},
            {'type': 'shell', 'text': 'echo hi'},
            {'type': 'click', 'x': -1, 'y': 2},
            {'type': 'click', 'x': 1280, 'y': 2},
            {'type': 'move', 'x': 2, 'y': 800},
            {'type': 'click', 'x': True, 'y': 2},
            {'type': 'click', 'x': 1.5, 'y': 2},
            {'type': 'click', 'x': 2, 'y': 2, 'extra': 0},
            {'type': 'type', 'text': ''},
            {'type': 'type', 'text': 'x' * 257},
            {'type': 'type', 'text': '\n'},
            {'type': 'type', 'text': 'é'},
            {'type': 'hotkey', 'keys': ['ctrl', 'invalid']},
            {'type': 'hotkey', 'keys': ['ctrl', 'ctrl']},
            {'type': 'press', 'key': []},
            {'type': 'scroll', 'amount': 0},
            {'type': 'scroll', 'amount': True},
            {'type': 'scroll', 'amount': 21},
        ]
        with self.desktop() as desktop:
            for action in actions:
                with self.subTest(action=action):
                    self.assert_code('invalid_action', lambda: desktop.execute(action))
    ...
    def test_typing_and_events_omit_input(self):
        ...
        self.assertEqual(
            ''.join(call[1] for call in self.backend.calls if call[0] == 'type'),
            'sensitive-sentinel',
        )
    ...
    def test_second_controller_rejected_and_lock_released(self):
        with self.desktop() as owner:
            ...
            owner.click(1, 1)
        with self.desktop() as next_owner:
            next_owner.click(2, 2)
        self.assertEqual(len(self.backend.calls), 2)

    def test_missing_checkpoint_has_bounded_timeout(self):
        with self.desktop() as desktop:
            self.assert_code(
                'checkpoint_timeout', lambda: desktop.wait('missing', lambda: False, timeout=0.01)
            )
