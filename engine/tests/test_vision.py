...
class VisionTests(unittest.TestCase):
    def setUp(self):
        self.anchor = Image.fromarray(np.random.default_rng(7).integers(0, 255, (19, 31), dtype=np.uint8))
    ...
    def test_region_resolves_only_the_declared_context(self):
        ...
        self.assertEqual(unique_match(self.scene, self.anchor, Box(120, 120, 200, 200)).box.left, 140)
    ...
    def test_missing_or_nondistinctive_anchor_rejects(self):
        ...
        self.assert_code('invalid_anchor', lambda: unique_match(self.scene, Image.new('L', (20, 20), 240)))

    def test_invalid_geometry_and_thresholds_reject(self):
        for box in [Box(-1, 0, 20, 20), Box(0, 0, 500, 20), Box(10, 0, 10, 20), Box(True, 0, 20, 20)]:
            self.assert_code('invalid_region', lambda: find_matches(self.scene, self.anchor, box))
        for threshold in [float('nan'), .1, 1.1, True]:
            self.assert_code('invalid_threshold', lambda: find_matches(self.scene, self.anchor, threshold=threshold))

    def test_amount_parsing_preserves_cents_without_guessing(self):
        for text, expected in [('$0.00', 0), ('$12.03', 1203), ('$9,876.54', 987654), ('$1234.05', 123405)]:
            self.assertEqual(parse_usd(text), expected)
        for text in ['', '—', '$O.00', '$1,23.45', '$01.23', '$12', '$12.3', '$12.345', '-$1.00', '$1.00 USD', '$ 1.00', '$1.00\n']:
    ...
    def test_actual_ocr_uses_memory_pipes_and_preserves_zeroes(self):
        ...
        ImageDraw.Draw(image).text((12, 10), '00340', fill='black',
    ...
    def test_blank_or_low_confidence_ocr_rejects(self):
        ocr = OCR(expected_data_hash=ENGLISH_SHA256)
        self.assert_code('ocr_uncertain', lambda: ocr.line(Image.new('RGB', (150, 50), 'white'), Box(0, 0, 150, 50)))
    ...
    def test_ocr_subprocess_deadline_is_enforced(self):
        ...
        with patch('interface_ai.vision.ocr.subprocess.run', side_effect=subprocess.TimeoutExpired('tesseract', .01)):
            self.assert_code('ocr_timeout', lambda: ocr.line(image, Box(0, 0, 100, 40), timeout=.01))
