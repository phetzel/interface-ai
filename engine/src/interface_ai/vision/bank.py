...
class BankVision:
    def __init__(self, *, event_sink=None):
        ...
        for name, entry in manifest.items():
            ...
            if hashlib.sha256(path.read_bytes()).hexdigest() != entry['sha256']:
                raise VisionError(
                    'anchor_integrity', 'Visual anchor hash does not match calibration'
                )
    ...
    def locate(self, name, image, region=None):
        if image.size != (1280, 800):
            raise VisionError(
                'unsupported_display', 'Recognition requires the calibrated 1280x800 display'
            )
        try:
    ...
    def identity(self, image, heading, member_id):
        ...
        if actual != member_id:
            raise VisionError(
                'identity_mismatch', 'Visible member identity does not match the requested member'
            )
    ...
    def savings_target(self, image, member_id):
        ...
        region = Box(
            heading.left, heading.top + 250, min(heading.left + 1080, image.width), image.height
        )
        savings = self.locate('savings-label', image, region)
        button = self.locate(
            'view-account', image, savings.relative((700, -10, 985, 45), image.size)
        )
