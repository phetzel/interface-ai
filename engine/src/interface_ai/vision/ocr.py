...
class OCR:
    def __init__(self, *, expected_data_hash, minimum_confidence=80):
        ...
        if hashlib.sha256(DATA.read_bytes()).hexdigest() != expected_data_hash:
            raise VisionError(
                'ocr_environment', 'Unexpected English OCR model; recalibration required'
            )
    ...
    def parse_tsv(self, text):
        try:
            ...
            if not rows or any(
                not math.isfinite(score) or not self.minimum_confidence <= score <= 100
                for score in scores
            ):
                raise VisionError(
                    'ocr_uncertain', 'OCR text is absent or below the confidence threshold'
                )
...
def parse_usd(text):
    ...
    if not isinstance(text, str) or not re.fullmatch(
        r'\$(?:0|[1-9][0-9]{0,2}(?:,[0-9]{3})*|[1-9][0-9]*)\.[0-9]{2}', text
    ):
