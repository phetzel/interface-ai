...
Routine events contain target/field names, boxes, scores, confidences, action types, and failure codes. They omit recognized text and typed values. Explicit synthetic results are written separately to `result.json`. Known-fixture captures are retained by the probe's synthetic export guard; this remains a demonstration restriction, not general sensitive-screen redaction.

M1-05 will add the validated manual artifact and interpreter, including the known member-not-found branch. M1-06 will run the full repeated acceptance gate. Passing this primitive harness does not complete either package or the full assignment.
