...
class Box:
    ...
    def checked(self, size):
        if (any(type(v) is not int for v in self.tuple()) or
                not 0 <= self.left < self.right <= size[0] or
                not 0 <= self.top < self.bottom <= size[1]):
...
def find_matches(image, template, region=None, *, threshold=.92):
    ...
    count, labels, stats, _ = cv2.connectedComponentsWithStats((scores >= threshold).astype(np.uint8), 8)
    matches = []
    if count > 65:
        raise VisionError('ambiguous_target', 'Too many visual candidates', candidateCount=count-1)
...
def unique_match(image, template, region=None, *, threshold=.92):
    ...
    if len(matches) != 1:
        raise VisionError('ambiguous_target', 'Visual target is not unique', candidateCount=len(matches))
    return matches[0]
