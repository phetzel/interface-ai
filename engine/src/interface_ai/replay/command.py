...
def replay(args, *, output_root=Path('/artifacts')):
    run_id = datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'-replay-'+uuid.uuid4().hex[:8]
    directory = output_root / run_id
    directory.mkdir()
    events = []
    started = time.monotonic()
    report = {'runId': run_id, 'modelCalls': 0, 'actionsCompleted': 0, 'phase': 'preflight',
              'packages': {name: importlib.metadata.version(name) for name in ['pydantic', 'opencv-python-headless', 'numpy', 'Pillow']}}
    interpreter = None
    ...
    try:
        ...
        try:
            data = strict_json(args.inputs_json) if args.inputs_json is not None else {'memberId': args.member_id}
        ...
        if platform.system() != 'Linux' or session['mode'] != 'bank':
            raise ReplayError('unsupported_environment', 'Replay requires the isolated bank desktop')
        report['sessionId'] = session['id']
        with Desktop(args.session if args.session is not None else session['id'],
                     timeout=bundle.capability.environment.totalTimeoutSeconds, event_sink=action_event) as desktop:
    ...
    print(json.dumps({'status': result.status, 'code': getattr(result, 'code', None),
                      'outcome': getattr(result, 'outcome', None), 'evidence': str(directory)}))
