...
M4-01 deliberately cannot type, navigate, record a capability or resume a workflow.
...
from interface_ai.desktop.session import RUNTIME, request_stop
...
class Probe:
    ...
    def request(self, operation, data):
        if not self.call_lock.acquire(blocking=False):
            raise DesktopError('busy', 'Probe already has a pending command')
        try:
            if self.status != 'running':
                raise DesktopError('invalid_transition', 'Probe is no longer active')
            future = Future()
