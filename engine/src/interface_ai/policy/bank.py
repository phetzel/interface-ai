...
REVIEWED_PATH = Path(__file__).resolve().parents[4] / 'capabilities/poc/savings-balance/capability.json'


def admit(bundle):
    if bundle.sha256 != APPROVED_SHA256:
        raise DesktopError('policy_artifact_denied', 'Capability revision has not been approved for this policy')
...
class BankPolicy:
    ...
    def authorize(self, action, desktop):
        ...
        self.pending = None
        runner = SimpleNamespace(cap=self.bundle.capability, bundle=self.bundle,
                                 guard=desktop.checkpoint, emit=lambda *a, **kw: None)
        ...
        def locate(name):
            try:
                ...
                if exc.code == 'target_missing':
                    return None
                raise DesktopError('policy_surface_denied', 'Policy target is ambiguous or unsupported') from None

        def inside(box):
            return box and box.left <= action['x'] < box.right and box.top <= action['y'] < box.bottom
        ...
        if action['type'] == 'click':
            ...
            if action == {'type': 'hotkey', 'keys': ['ctrl', 'a']}:
                ...
                return
            if action['type'] == 'type' and self.keyboard in ('field', 'selected') and re.fullmatch('[0-9]{5}', action['text']):
                self.pending = 'typed'
                return
            if action == {'type': 'press', 'key': 'enter'} and self.keyboard == 'typed':
                return
        self.clear()
        raise DesktopError('policy_operation_denied', 'Operation is outside the reviewed read-only workflow')
