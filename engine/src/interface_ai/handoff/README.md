...
M4-01 adds separately authenticated `/probe/*` requests on this same port. Its bounded worker reserves input while waiting for the host provider; the panel shows that phase and keeps Stop available. It does not offer takeover/resume or lookup start during a probe. The probe and host retain their own evidence, and panel status reports unknown provider usage instead of zero. Shared lifecycle remains M4-02 work; see [the probe guide](../../../../docs/M4_01_PROVIDER_PROBE.md).
